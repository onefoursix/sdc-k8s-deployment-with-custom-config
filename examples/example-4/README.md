...
An example Job is defined [here](https://github.com/onefoursix/sdc-k8s-deployment-with-custom-config/blob/master/examples/example-4/sdc-stage-libs-job.yaml);. It spins up a [BusyBox](https://www.busybox.net/about.html) container that uses shell commands to download and extract the stage libs, and writes them to the directory <code>/streamsets-libs</code> in the Persistent Volume.  As this Job launches the first Pod to use the PVC, it will be able to take advantage of the <code>ReadWriteOnce</code> AccessMode and write to the PV. 
